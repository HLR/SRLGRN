...
# import sys
# sys.path.append('../')
# from end_to_end_test.test_config import TEST_CONFIG
...
def evaluate():
    ...
    logger.info("  Batch size = %d", cfg.predict_batch_size)
    ...
    dataloader = DataLoader(data, sampler=sampler, batch_size=cfg.predict_batch_size)
...
if __name__ == "__main__":
    ...
    model_state_dict = torch.load(cfg.para_sele_ckpt)
    model = BertForSequenceClassification.from_pretrained(cfg.para_sele_path, state_dict=model_state_dict, num_labels=2)
    model.cuda()
    model = torch.nn.DataParallel(model)
    tokenizer = BertTokenizer.from_pretrained(cfg.para_sele_path, do_lower_case=cfg.do_lower_case)

    processor = DataProcessor()
    label_list = processor.get_labels()
    source_data, dataframe = get_dataframe(cfg.test_input_file)

    examples = processor.create_examples(dataframe, 'test')
    features = convert_examples_to_features(examples, label_list, cfg.para_sele_max_seq_length, tokenizer, verbose=True)

    score, _, _ = evaluate()
    get_dev_paras(source_data, score, cfg.para_sele_predict_output_file)
