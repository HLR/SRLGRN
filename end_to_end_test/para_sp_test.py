...
import sys
sys.path.append('../')
from para_sp.model import BertForSequentialSentenceSelector
...
class SequentialSentenceSelector:
    def __init__(self,
        ...
        #     args.para_sp_bert_model, do_lower_case=args.do_lower_case)
        # model_state_dict = torch.load(args.para_sp_ckpt)
        # self.model = BertForSequentialSentenceSelector.from_pretrained(args.para_sp_bert_model, state_dict=model_state_dict)

        self.tokenizer = BertTokenizer.from_pretrained(
            args.para_sp_path, do_lower_case=args.do_lower_case)
        model_state_dict = torch.load(args.para_sp_ckpt)
        self.model = BertForSequentialSentenceSelector.from_pretrained(args.para_sp_path, state_dict=model_state_dict)
    ...
    def predict(self,
        ...
        eval_examples = self.processor.get_train_examples(args.para_sp_predict_input_file)
        eval_features = convert_examples_to_features(
            eval_examples, args.para_sp_max_seq_length, args.max_sent_num, args.max_sf_num, self.tokenizer)
        ...
        with open(args.final_output_sp_file, "w") as writer:
...
def main():
    ...
    print('finish supporting fact reasoning!')
