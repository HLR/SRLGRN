                ...
                if is_training:
                    ...
                    if not is_impossible:
                        ...
                        if actual_text.find(cleaned_answer_text) == -1:
                            #### chen add this part
                            #if cleaned_answer_text is not 'yes' and cleaned_answer_text is not 'no':
                            #### chen add this part done
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:  # this is a non-null prediction
                ...
                tok_text = tok_text.replace("▁", " ")
                # print('tok_text2: ', tok_text)

                # Clean whitespace
                tok_text = tok_text.strip()
                # tok_text = " ".join(tok_text.split())
                # print('tok_text3: ', tok_text)

                orig_text = " ".join(orig_tokens).lower()
...
# tokenizer = BasicTokenizer(do_lower_case=reader_cfg.do_lower_case)
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    # reader_cfg = READER_CONFIG()
    # tokenizer = RobertaTokenizer.from_pretrained(reader_cfg.bert_model, do_lower_case=reader_cfg.do_lower_case)
    ...
    if len(orig_ns_text) != len(tok_ns_text):
        tok_ns_text = tok_ns_text.replace('▁','')
    ...
    # print('start postion: ', start_position , "  end postion: ", end_position)
