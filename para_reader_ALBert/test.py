...
def main():
    ...
    # # model = BertForQuestionAnsweringConfidence.from_pretrained(state_dict=model_state_dict)
    # tokenizer = AlbertTokenizer.from_pretrained(reader_cfg.bert_model, do_lower_case=reader_cfg.do_lower_case)

    model_state_dict = torch.load(reader_cfg.load_ckpt_path)
    model = BertForQuestionAnsweringConfidence.from_pretrained(reader_cfg.bert_model, state_dict=model_state_dict)  ## still not sure
    tokenizer = AlbertTokenizer.from_pretrained(reader_cfg.bert_model, do_lower_case=reader_cfg.do_lower_case)
