...
### Chen: The model entry BertForQuestionAnsweringConfidence is the same as the other models.
### Chen: In this file, I have a SRLGRN class to do the para_reader with both lm and gnn.
class BertForQuestionAnsweringConfidence(nn.Module):
    def __init__(self, config):
        super(BertForQuestionAnsweringConfidence, self).__init__()
        self.config = config
        ...
        self.graph_store_res = nn.ModuleList()
        ### chen: default the num_gnn_layers == 1
        for _ in range(self.config.num_gnn_layers):
            self.graph_store_res.append(SRLGRN(self.config.q_attn, config))

        self.para_attention = Para_Attention(input_dim=config.hidden_dim*2,
    ...
    def forward(self, batch, return_yp):
        ...
        # chen: split the query encoding for state use.
        ...
        ### chen: default the gnn layer == 1 so the list has only one layer's result.
        for l in range(self.config.num_gnn_layers):
            ...
            sent_logit, sent_prediction, ent_logit = self.graph_store_res[l](batch, input_state, query_vec)
        ...
        input_state, _ = self.para_attention(input_state, graph_state, graph_mask.squeeze(-1))
...
class GNN(nn.Module):
    ...
    def forward(self, input_state, adj, node_mask=None, query_vec=None):
        ...
        for i in range(self.n_type):
            ...
            if self.q_attn:
                para = F.relu(torch.matmul(query_vec, self.qattn_W1[i]))
                para = torch.sigmoid(torch.matmul(para, self.qattn_W2[i]))
                a_input = a_input * para[:, None, None, :]
    ...
    def __init__(self, in_dim, hid_dim, n_head, q_attn, config):
        ...
        for i in range(n_head):
            self.attn_funcs.append(
                GNN(in_dim=in_dim, out_dim=hid_dim // n_head, config=config, q_attn=q_attn, head_id=i))
...
### chen: from BertLayerNorm source code
...
class SRLGRN(nn.Module):
    def __init__(self, q_attn, config):
        super(SRLGRN, self).__init__()
        self.config = config
        self.hidden_dim = config.hidden_dim

        if self.config.q_update:
            self.gnn_linear = nn.Linear(self.hidden_dim*2, self.hidden_dim)
        else:
            self.gnn_linear = nn.Linear(self.config.input_dim, self.hidden_dim*2)

        if self.config.q_update:
            self.gnn = AttentionLayer(self.hidden_dim, self.hidden_dim, config.num_gnn_heads, q_attn=q_attn, config=self.config)
            ...
            self.gnn = AttentionLayer(self.hidden_dim*2, self.hidden_dim*2, config.num_gnn_heads, q_attn=q_attn, config=self.config)
    ...
    def forward(self, batch, input_state, query_vec):
        ...
        if self.config.q_update:
            graph_state = self.gnn_linear(torch.cat([para_state, sent_state, ent_state], dim=1)) # N * (max_para + max_sent + max_ent) * d
            graph_state = torch.cat([query_vec.unsqueeze(1), graph_state], dim=1)
        else:
            graph_state = self.gnn_linear(query_vec)
        ...
        graph_state = self.gnn(graph_state, graph_adj, node_mask=node_mask, query_vec=query_vec) # N x (1+max_para+max_sent) x d
        ent_state = graph_state[:, 1+max_para_num+max_sent_num:, :]

        gnn_logit = self.sent_mlp(graph_state[:, :1+max_para_num+max_sent_num, :]) # N x max_sent x 1
        para_logit = gnn_logit[:, 1:1+max_para_num, :].contiguous()
        sent_logit = gnn_logit[:, 1+max_para_num:, :].contiguous()
...
class Para_Attention(nn.Module):
    def __init__(self, input_dim, memory_dim, hid_dim, dropout):
        super(Para_Attention, self).__init__()
